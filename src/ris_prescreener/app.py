    ...
    for index, group in enumerate(st.session_state["groups"]):
        ...
        with left:
            group["name"] = st.text_input(
                "Group name", group["name"], key=f"group_name_{index}"
            )
        with middle:
    ...
    if st.button("Run screening", type="primary", disabled=uploaded is None):
        if uploaded is None:
            ...
            try:
                ...
                named_groups = [group for group in configured_groups if group["name"]]
                names = [group["name"].casefold() for group in named_groups]
                if len(names) != len(set(names)):
                    raise ValueError("Group names must be unique.")
                if any(not group["terms"] for group in named_groups):
                    raise ValueError("Every group needs at least one keyword.")
                if not named_groups:
                    raise ValueError("Add at least one named keyword group.")
                groups = {group["name"]: group["terms"] for group in named_groups}
                required = [
                    group["name"] for group in named_groups if group["required"]
                ]
                result = screen_ris(
                    temp_path,
                    groups,
